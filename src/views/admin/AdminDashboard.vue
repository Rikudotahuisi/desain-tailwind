<template>
  <div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            >
              <i
                :class="
                  stat.change >= 0 ? 'fas fa-arrow-up' : 'fas fa-arrow-down'
                "
              ></i>
      ...
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
        ...
        <div class="space-y-3">
          <div
            ...
            :key="schedule.time"
            class="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition"
          >
            <div class="w-16 text-sm font-medium text-slate-600 pt-0.5">
              {{ schedule.time }}
            </div>
            <div class="flex-1">
              <p class="text-sm font-medium text-slate-900">
    ...
    <!-- Statistics Table -->
    <div
      class="mt-6 bg-white rounded-2xl p-6 shadow-sm border border-slate-200"
    >
      <div class="flex items-center justify-between mb-4">
        <h3 class="font-bold text-slate-900">Statistik Bulanan</h3>
        <select
          class="text-sm border border-slate-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-teal-500/20"
        >
          <option>2026</option>
          <option>2025</option>
        </select>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-200">
              <th class="text-left py-3 px-2 font-semibold text-slate-600">
                Bulan
              </th>
              <th class="text-right py-3 px-2 font-semibold text-slate-600">
                Pasien
              </th>
              <th class="text-right py-3 px-2 font-semibold text-slate-600">
                Dokter
          ...
          <tbody>
            ...
            >
              ...
              <td class="text-right py-3 px-2 text-slate-600">
                {{ stat.patients }}
              </td>
              <td class="text-right py-3 px-2 text-slate-600">
                {{ stat.doctors }}
              ...
              <td class="text-right py-3 px-2 font-medium text-teal-600">
                Rp {{ stat.revenue }}
              </td>
            </tr>
          </tbody>
