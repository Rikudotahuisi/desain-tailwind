<template>
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-sm text-slate-500">
        Kelola jadwal praktek dokter di ASSYIFA Hospital
      </p>
    </div>
    <button
  ...
  >
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-slate-50 border-b border-slate-200">
            <th class="text-left py-3.5 px-6 font-semibold text-slate-600">
              Dokter
            </th>
            <th class="text-left py-3.5 px-6 font-semibold text-slate-600">
              Hari Aktif
            ...
            <th class="text-right py-3.5 px-6 font-semibold text-slate-600">
              Aksi
            </th>
          </tr>
        </thead>
        <tbody>
          <tr
            v-for="item in doctorScheduleList"
            :key="item.id"
          ...
          >
            ...
            <td class="py-4 px-6 text-right">
              <button
                @click="router.push(`/admin/schedules/edit/${item.id}`)"
                class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-teal-600 hover:bg-teal-50 transition"
              >
    ...
    <div
      v-if="doctorScheduleList.length === 0"
...
function handleDeleteSchedule(doctorId: number) {
  if (confirm("Hapus seluruh jadwal praktek dokter ini?")) {
    getSchedulesForDoctor(doctorId).forEach((entry) =>
