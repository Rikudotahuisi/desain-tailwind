<template>
  <div class="min-h-screen bg-slate-50" style="padding-top: 140px">
        ...
        <div class="flex-1">
          ...
          <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            >
              <!-- Image Placeholder -->
              <div
                class="relative h-48 bg-gradient-to-br from-teal-100 to-teal-200 flex items-center justify-center"
              >
                <i :class="[item.icon, 'text-6xl text-teal-500/50']"></i>
                <span
                  class="absolute top-3 right-3 bg-teal-500 text-white text-xs px-2.5 py-1 rounded-full font-semibold"
                >
                  {{ item.category }}
                </span>
                <div
    ...
    >
      <div
        class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4"
      >
        <div
        ...
        <div class="p-6 space-y-5">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <div
                v-for="test in selectedItem.tests"
                :key="test"
          ...
          >
            ...
            <div class="flex gap-3">
              <router-link
                to="/register"
                class="rounded-full bg-teal-500 px-6 py-2.5 font-semibold text-white transition hover:bg-teal-600"
              >
                <i class="fas fa-calendar-check mr-2"></i>Booking
              </router-link>
              <button
                @click="selectedItem = null"
                class="rounded-full border border-slate-200 px-6 py-2.5 font-semibold text-slate-600 transition hover:bg-slate-50"
              >
                Tutup
              </button>
            </div>
...
const genderFilters = ["Semua", "Laki-laki", "Wanita"];
...
const activeFilterCount = computed(() => {
  ...
  if (selectedGenders.value.length > 1)
    count += selectedGenders.value.length - 1;
  if (selectedCategories.value.length > 1)
    count += selectedCategories.value.length - 1;
  if (selectedPopular.value.length > 1)
    count += selectedPopular.value.length - 1;
  return count;
});
...
const filteredItems = computed(() => {
  ...
  if (
    !selectedPopular.value.includes("Semua") &&
    selectedPopular.value.length > 0
  ) {
    filtered = filtered.filter((item) =>
      selectedPopular.value.includes(item.name),
    );
  }
  ...
  if (
    !selectedGenders.value.includes("Semua") &&
  ...
  if (
    !selectedCategories.value.includes("Semua") &&
