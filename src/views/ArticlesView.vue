  ...
  <div class="min-h-screen bg-slate-50" style="padding-top: 140px">
    ...
    <div class="mx-auto max-w-[1400px] px-6 lg:px-16 -mt-6">
      ...
      <div
        ...
        class="mt-6 rounded-2xl overflow-hidden bg-white shadow-lg hover:shadow-xl transition-all duration-300"
      >
        <div class="grid grid-cols-1 lg:grid-cols-2">
          <div class="relative h-64 lg:h-auto overflow-hidden">
            ...
            />
            <span
              class="absolute top-4 left-4 rounded-full bg-teal-500 px-3 py-1 text-xs font-semibold text-white shadow-lg"
            >
          ...
          <div class="flex flex-col justify-center p-6 lg:p-8">
            <div class="flex flex-wrap items-center gap-2 text-sm">
              <span
                class="rounded-full bg-teal-50 px-3 py-1 text-xs font-medium text-teal-600"
              >
            ...
            <h3 class="mt-3 text-2xl font-bold text-slate-900 lg:text-3xl">
      ...
      <!-- Articles Grid -->
      <div
        class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 lg:grid-cols-3"
      >
        <router-link
        ...
        >
          <div class="relative h-44 overflow-hidden bg-slate-200 md:h-48">
            ...
            />
            <span
              class="absolute top-2 left-2 rounded-full bg-teal-500 px-2 py-0.5 text-[9px] font-semibold text-white shadow-md md:top-3 md:left-3 md:px-2.5 md:py-0.5 md:text-[10px]"
            >
              {{ article.category }}
            </span>
            <span
              class="absolute bottom-2 right-2 rounded-full bg-black/60 px-2 py-0.5 text-[9px] text-white backdrop-blur-sm md:bottom-3 md:right-3 md:px-2.5 md:py-0.5 md:text-[10px]"
            >
          ...
          <div class="p-4 md:p-5">
            <div
              class="flex items-center gap-1.5 text-[10px] text-slate-500 md:text-xs"
            >
              <span
                ><i class="far fa-calendar-alt mr-1"></i
                >{{ article.date }}</span
              ...
              <span
                ...
                >{{ article.author.split(" ").slice(0, 2).join(" ") }}</span
              >
            </div>
            <h3
              class="mt-1.5 text-sm font-bold text-slate-900 group-hover:text-teal-600 transition-colors line-clamp-2 md:text-base"
            >
              {{ article.title }}
            </h3>
            <p class="mt-1 text-xs text-slate-600 line-clamp-2 md:text-sm">
              {{ article.excerpt }}
            </p>
            <div
              class="mt-2 flex items-center justify-between border-t border-slate-100 pt-2 md:mt-3 md:pt-3"
            >
              <span
              ...
              <div
                class="flex items-center gap-1.5 text-[10px] text-slate-400 md:gap-2 md:text-xs"
              >
                <button
                  @click.stop.prevent
      ...
      <div
        ...
        class="py-12 text-center md:py-16"
      >
        ...
        <p class="text-sm text-slate-500">
          Coba dengan kategori atau kata kunci lain
        </p>
        <button
          @click="resetFilters"
          class="mt-3 rounded-full bg-teal-500 px-5 py-1.5 text-sm font-semibold text-white transition hover:bg-teal-600 md:mt-4 md:px-6 md:py-2"
        >
          Reset Filter
        </button>
      </div>
      ...
      <!-- Pagination -->
      <div
        class="mt-8 flex items-center justify-center gap-1.5 pb-10 md:mt-12 md:gap-2 md:pb-12"
      >
        <button
          class="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs text-slate-600 transition hover:bg-slate-50 hover:-translate-y-0.5 md:px-3 md:py-1.5 md:text-sm"
        >
          <i class="fas fa-chevron-left text-[10px] md:text-xs"></i>
        </button>
        <button
          class="rounded-full bg-teal-500 px-3 py-1 text-xs font-semibold text-white shadow-lg shadow-teal-500/20 md:px-3.5 md:py-1.5 md:text-sm"
        >
          1
        </button>
        <button
          class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-600 transition hover:bg-slate-50 hover:-translate-y-0.5 md:px-3.5 md:py-1.5 md:text-sm"
        >
          2
        </button>
        <button
