<template>
  <div class="min-h-screen bg-slate-50" style="padding-top: 160px">
    <div class="mx-auto max-w-[1400px] px-6 lg:px-16 -mt-6 pb-16">
      ...
      <!-- Grid Dokter -->
      <div
        class="grid grid-cols-1 gap-6 items-start md:grid-cols-2 xl:grid-cols-3"
      >
        <div
        ...
        >
          ...
          <div class="mt-4">
            ...
            >
              ...
              Praktik Hari Ini • {{ todaySlot(doctor)?.startTime }} -
              {{ todaySlot(doctor)?.endTime }}
            </span>
            <span
              v-else
          ...
          <div class="mt-4 grid grid-cols-7 gap-1">
            ...
            >
              {{ d.slice(0, 3) }}
              <i
                v-if="isActiveDay(doctor.id, d)"
                class="fas fa-check text-[8px] mt-0.5"
              ></i>
          ...
          >
            {{
              expandedIds.has(doctor.id)
                ? "Sembunyikan Jadwal"
                : "Lihat Jadwal Lengkap"
            }}
            <i
              class="fas fa-chevron-down transition-transform"
              :class="expandedIds.has(doctor.id) ? 'rotate-180' : ''"
            ></i>
          </button>

          <!-- Detail Jadwal Lengkap -->
          <div
            v-if="expandedIds.has(doctor.id)"
            class="mt-2 space-y-1 rounded-xl bg-slate-50 p-3"
          >
            <div
              v-for="schedule in getSchedulesForDoctor(doctor.id).filter(
                (s) => s.active,
              )"
              :key="schedule.day"
              class="flex items-center justify-between rounded-lg px-2 py-1.5 text-sm"
              :class="schedule.day === today ? 'bg-white shadow-sm' : ''"
            >
              <span
                class="font-medium"
                :class="
                  schedule.day === today ? 'text-teal-600' : 'text-slate-600'
                "
              >
                {{ schedule.day }}
              </span>
      ...
      <div v-if="filteredDoctors.length === 0" class="py-16 text-center">
        <div class="text-6xl mb-4">
          <i class="fas fa-user-md text-slate-300"></i>
        </div>
        ...
        <button
          @click="resetFilters"
          class="mt-4 rounded-full bg-teal-500 px-6 py-2 text-sm font-semibold text-white transition hover:bg-teal-600"
        >
          Reset Filter
        </button>
      </div>
...
const isActiveDay = (doctorId: number, day: string) =>
  !!scheduleForDay(doctorId, day);
...
const practicesToday = (doctor: Doctor) =>
  getTodaySchedulesForDoctor(doctor.id).length > 0;
